...
func (c *WaspClient) PutChainRecord(rec *chain.ChainRecord) error {
	return c.do(http.MethodPost, routes.PutChainRecord(), model.NewChainRecord(rec), nil)
}

// GetChainRecord fetches a Record by address
func (c *WaspClient) GetChainRecord(chainid coretypes.ChainID) (*chain.ChainRecord, error) {
...
func (c *WaspClient) GetChainRecordList() ([]*chain.ChainRecord, error) {
	var res []*model.ChainRecord
	if err := c.do(http.MethodGet, routes.ListChainRecords(), nil, &res); err != nil {
		return nil, err
	}
	list := make([]*chain.ChainRecord, len(res))
	for i, bd := range res {
		list[i] = bd.Record()
	}
	return list, nil
